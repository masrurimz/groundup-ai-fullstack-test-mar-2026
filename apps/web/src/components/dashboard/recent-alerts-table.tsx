          ...
          <TableBody className="divide-y divide-slate-200 dark:divide-slate-700">
            {alerts.slice(0, 8).map((alert) => (
              <TableRow
                ...
                className="relative cursor-pointer transition-colors hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                <TableCell className="px-6 py-4 pl-8">
                  <div
                    className={`absolute inset-y-0 left-0 w-[3px] ${severityStripe(alert.severity)}`}
                  />
                  <div className="flex flex-col">
                    <p className="font-medium text-slate-900 dark:text-slate-50">{alert.title}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{alert.machine}</p>
                  </div>
                </TableCell>
